...
This page has a compact table of the available data and some longer notes and
examples.
...
| --- | --- | ---: | --- |
| Synthetic data | `sphere()`, `ball()`, `helix()`, `swiss_roll()`, `s_curve()` | User-chosen | Local generation, no download. |
| Distill t-SNE simulations | `gaussian_data()`, `two_clusters_data()`, `random_walk()`, `trefoil_data()` | User-chosen | R translations of the Distill demo data generators. |
| PaCMAP and topological examples | `s_curve_hole()`, `curve2d()`, `taspheres()` | User-chosen | Local generation, no download. |
| Mammoth point clouds | `download_mammoth10k()`, `download_mammoth50k()` | 10,000 or 50,000 | Downloads JSON point-cloud data. |
| MNIST and relatives | `download_mnist()`, `download_fashion_mnist()`, `download_kuzushiji_mnist()`, `download_qmnist()` | 70,000 or 120,000 | Data frame by default; optional matrix/list return for MNIST and QMNIST. |
| CIFAR-10 | `download_cifar10()`, `show_cifar()` | 60,000 | Data frame by default; optional matrix/list return. |
| Small NORB | `download_norb_small()`, `show_norb_object()` | 48,600 | Split downloads and optional matrix/list return are available. |
| 20 Newsgroups | `download_twenty_newsgroups()` | 18,846 | Text data; use packages such as `tm` or `tidytext` for preprocessing. |
| Frey and Olivetti faces | `frey_faces()`, `olivetti_faces()` | 1,965 and 400 | Requires the suggested `RnavGraphImageData` package. |

For MNIST, QMNIST, CIFAR-10, and Small NORB, you can return a list containing
the matrix of the numeric data and any labels separately. Supply
`as = "matrix"`.
